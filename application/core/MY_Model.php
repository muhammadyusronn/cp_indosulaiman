...
class MY_Model extends CI_Model
{
    ...
    {
        return $this->db->insert_batch($this->data['table_name'], $data);
    }
    ...
    public function get_data_join($tables, $jcond,$cond=null,$type='default')
    {
        $this->db->select('*');
        ...
        if (is_array($cond))
            $this->db->where($cond);
        if($type=='array'){
            return $this->db->get($this->data['table_name'])->result_array();
        }else{
